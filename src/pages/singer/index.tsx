                ...
                <div className="tag_area">
                    {
                        areaList.map((item:{title:string,key:number}) =>{
                            return <div key={item.key} className="tag_item" style={{color: item.key===data.area?'#ffcd32':'#fff'}}
                                    onClick={()=>{checkArea(item.key)}}>{item.title}</div>
                        })
                    }
                </div>
                <div className="tag_type">
                    {
                        typeList.map((item:{title:string,key:number}) =>{
                            return <div key={item.key} className="tag_item" style={{color: item.key===data.type?'#ffcd32':'#fff'}}
                                        onClick={()=>{checkType(item.key)}}>{item.title}</div>
                        })
                    }
                </div>
            </div>
